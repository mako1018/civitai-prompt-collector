...
def try_print_json(resp):
    try:
        ...
        if "application/json" in ct or "api" in resp.url.lower() or "models" in resp.url.lower():
            try:
                data = resp.json()
            except Exception:
                return
...
with sync_playwright() as pw:
    ...
    print("Browser opened. You have 30 seconds to interact with the page (scroll/click).")
